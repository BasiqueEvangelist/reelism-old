...
public class TunnelingEnchantment extends Enchantment {
    ...
    public static void register() {
        PlayerBlockBreakEvents.AFTER.register((World world, PlayerEntity player, BlockPos pos, BlockState state,
                @Nullable BlockEntity blockEntity) -> {
            ItemStack main = player.getMainHandStack();
            Map<Enchantment, Integer> ench = EnchantmentHelper.get(main);
            if (ench.containsKey(ReeEnchantments.TUNNELING) && main.getMiningSpeedMultiplier(state) > 1.0F
